...
const Chat: React.FC = () => {
        ...
        if (data.text) {
          ...
          setMessages(prevMessages => {
            ...
            if (lastMessage && lastMessage.sender === 'bot') {
              ...
              // In case no bot message exists, add one
  ...
  // When sending a message, add the user's message and a placeholder for the bot's response.
  // The bot's response will be updated via incoming WebSocket messages.
  const sendMessage = async () => {
    if (input.trim() === '') return;
    ...
    // Trigger the backend process that starts the streaming response.
  ...
  const handleFeedbackSubmit = () => {
    ...
    setIsFeedbackOpen(false);
    setFeedback('');
    // Additional logic to submit feedback can be added here.
  };

  const createNewChat = () => {
    console.log('New chat created');
    // Additional new chat logic can be added here.
  };
